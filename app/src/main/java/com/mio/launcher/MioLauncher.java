...
import com.mio.launcher.adapter.*;
            ...
            if (MioUtils.isNetworkConnected(this)) {
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        String str = "";
                        try {
                            HttpURLConnection con = (HttpURLConnection) new URL("https://" + ip + "/titles/publicMio.txt").openConnection();
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        try {
                            HttpURLConnection con = (HttpURLConnection) new URL("https://" + ip + "/titles/zz.txt").openConnection();
                            con.setConnectTimeout(5000);
                            InputStream in = con.getInputStream();
                            BufferedReader bfr = new BufferedReader(new InputStreamReader(in));
                            String temp = null;
    ...
    //            startActivity(new Intent(MioLauncher.this, MioPack.class));
                ...
                if (null != file && file.exists()) {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                        Uri contentUri = FileProvider.getUriForFile(this, "com.mio.launcher.provider", file);
